...
export default function SignUpButton(props:{text:string, func?:Function}){
    ...
    return(
        ...
        <div className="p-1 border-2 border-white md:flex flex-row h-16  hidden">
         <button onClick={()=>{if(success){setSuccess(false)}else{setActive(!active)}; }} className={`hover:bg-transparent transition-all duration-500 w-56 pt-3 pb-2 md:text-[20px] text-[16px]  ${success? ' text-green-400 ' : active ? 'text-black' : 'text-black'}  ${active ? 'bg-transparent' : 'bg-white'} font-semibold`}>{`${active ? 'No spam, ever :p' : success? 'Success ^w^' : 'Sign up for updates!'}`}</button>
         <div className={` transition-all overflow-hidden duration-500 ${active ? 'w-0.5' : 'w-0'} h-full bg-white`}></div>
            ...
            setSuccess(true)}} className={` md:text-[20px] text-[16px] transition-all overflow-hidden max-h-full duration-700 ${active ? 'w-80' : 'w-0'} pt-1`}>
                <label htmlFor=""><input placeholder={`${success ? 'success!': 'lia@emelia.pl'}`} onChange={(e)=>{setEmail(e.target.value)}} className={`h-full pr-2 w-72  pl-6 bg-transparent  focus:outline-none`} type="email"/></label>
        ...
        <div className="p-1 border-2 border-white md:hidden flex-col h-max  block">
         <button onClick={()=>{if(success){setSuccess(false)}else{setActive(!active)}; }} className={`hover:bg-transparent transition-all duration-500 w-64 pt-3 pb-2 md:text-[20px] text-[16px]  ${success? ' text-green-400 ' : active ? 'text-black' : 'text-black'}   ${active ? 'bg-transparent' : 'bg-white'} font-semibold`}>{`${active ? 'No spam, ever :p' : success? 'Success ^w^' : 'Sign up for updates!'}`}</button>
